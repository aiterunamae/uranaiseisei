...
if vertex_ai_project_id:
    ...
    if st.session_state.get('show_readme', False):
        with st.expander("📖 汎用占い生成アプリ 使い方ガイド", expanded=True):
            st.markdown("""
# 🔮 汎用占い生成アプリ

## 概要
このアプリは、GoogleのAIモデル（Gemini）を使用して、様々な種類の占いを生成するツールです。
キーワードやルールをカスタマイズして、独自の占いスタイルを作成できます。

## 🚀 クイックスタート

### 1. 初期設定
1. **キーワードCSVのアップロード**（サイドバーの「🎯 プリセット」タブ内）
   - ハウスキーワード.csvなどのCSVファイルをアップロード
   - 複数のファイルを同時にアップロード可能

2. **システムプロンプトの入力**
   - 占い師のキャラクターや占いのスタイルを定義

### 2. 基本的な使い方
1. **質問の入力**
   - テキスト入力：単一の質問を直接入力
   - CSVファイル入力：複数の質問を個別に処理
   - CSV連続モード：複数の質問を関連付けて処理
...
### プリセットのエクスポート/インポート
- JSONファイルでプリセットの保存・共有が可能

## ⚙️ AIモデル設定

### 利用可能なモデル
- **gemini-2.0-flash**：高速・軽量
- **gemini-2.5-flash**：バランス型（思考機能付き）
- **gemini-2.5-pro**：高精度（思考機能付き）

### Thinking Budget（思考トークン）
Gemini 2.5モデルでは、AIが「考える」ためのトークン数を指定できます。
値が大きいほどより深い思考が可能です。
...
- キーワードCSVは事前にアップロードが必要
- 大量の組み合わせを生成すると時間がかかる場合があります
- APIの利用制限に注意してください

## 🔧 トラブルシューティング

### エラーが出る場合
- キーワードCSVがアップロードされているか確認
- システムプロンプトが入力されているか確認
